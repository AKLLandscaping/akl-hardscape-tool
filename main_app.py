...
from utils import (
    ...
    load_garden_wall_data, load_extras_data,
...
if section == "Walkway":
    st.subheader("🚶 Walkway Calculator")
    sqft = st.number_input("Square Feet", min_value=0)

    # Load and filter products
    paver_data = load_paver_data()
    product_names = paver_data.iloc[:, 1].dropna().unique()
    selected_product = st.selectbox("Choose Material", product_names)
    ...
    rate = st.selectbox("Hourly Rate", [35, 40, 45, 50, 55, 60], index=2)
